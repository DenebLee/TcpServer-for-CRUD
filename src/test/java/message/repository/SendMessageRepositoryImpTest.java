...
class SendMessageRepositoryImpTest extends TestRepositorySetUp {
    ...
    void should_delete_all() throws SelectException, InsertException, DeleteException {
        // given
        List<SendMessage> expected = new ArrayList<>();
        System.out.println(id);
        for (int i = 0; i < 100; i++) {
    ...
    void should_send_update() throws SelectException, UpdateException {
        // given
        SendMessage originalData = new SendMessage(MessageType.SMS, MessageStatus.SELECTED, 0, id, "안녕하세요");
        sendToTelecomMessageRepository.save(originalData);
        long send_id = originalData.getSend_id();
        SendMessage expected = new SendMessage(MessageType.SMS, MessageStatus.SELECTED, 0, id, "수정된 값");
